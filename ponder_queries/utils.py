...
def test_results(q_num: int, result_df: PandasDF):
    with CodeTimer(name=f"Testing result of Ponder Query {q_num}", unit="s"):
        import pandas as pd
...
def run_query(q_num: int, query: Callable):
    @linetimer(name=f"Overall execution of Ponder Query {q_num}", unit="s")
    def run():
        with CodeTimer(name=f"Get result of Ponder Query {q_num}", unit="s"):
        ...
        if LOG_TIMINGS:
            append_row(
                solution="Ponder", version=ponder.__version__, q=f"q{q_num}", secs=secs
            )
        else:
            pass
